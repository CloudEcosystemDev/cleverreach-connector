...
async function insertReceiver(msg, cfg, categories) { // , recordUid
  try {
    ...
    let groupId = cfg.group_id || msg.data.group_id;
...
async function getReceiversByGroupId(msg, cfg, snapshot) {
  try {
    // First get all groups because we also need the group name
    const groups = await getGroups(cfg);
    const groupsLength = groups.length;

    let groupId = cfg.group_id || msg.group_id;
    let groupName = cfg.group_name || msg.group_name;
